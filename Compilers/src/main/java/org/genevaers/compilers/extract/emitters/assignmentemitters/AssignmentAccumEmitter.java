...
public class AssignmentAccumEmitter extends AssignmentEmitter{

    @Override
    public LTFileObject makeLTEntry(ExtractBaseAST lhs, ExtractBaseAST rhs) {
        LTFileObject ltEntry = ((ColumnAST)lhs).getAccumLtEntry(((NumericAccumulator)rhs).getAccumulatorName());
        ((LogicTableNameF1)ltEntry).getArg().setLogfileId(getLtEmitter().getFileId());        
        return ltEntry;
    }    
}
