...
public class FormatGT extends FormatBaseAST implements EmittableFormatASTNode{
    FormatGT() {
        type = Type.GT;
    }
    ...
    public CalcStackEntry emit(boolean invert) {
        inverted = invert;
        if(invert) {
            return emitIntegerCodeEntry(CalcStackOpcode.CalcStackBranchLE, super.emit(invert));
        } else {
            return emitIntegerCodeEntry(CalcStackOpcode.CalcStackBranchGT, super.emit(invert));
        }
    }

    public void setCalcStackEntryValue(int v) {
        ((CalcStackIntegerEntry)calcStackEntry).setValue(v);
    }
}
