...
public class FormatGE extends FormatBaseAST implements EmittableFormatASTNode{
    FormatGE() {
        type = Type.GE;
    }
    ...
    public CalcStackEntry emit(boolean invert) {
        inverted = invert;
        if(invert) {
            return emitIntegerCodeEntry(CalcStackOpcode.CalcStackBranchLT, super.emit(invert));
        } else {
            return emitIntegerCodeEntry(CalcStackOpcode.CalcStackBranchGE, super.emit(invert));
        }
    }
}
